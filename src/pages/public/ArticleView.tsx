    ...
    <div className="container py-8 md:py-12 max-w-4xl">
      ...
      <div className="rounded-lg border bg-card p-6 md:p-8 mb-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-6 py-4 border-y text-sm">
          ...
          {article.section && (
            <div>
              <div className="text-[0.65rem] uppercase tracking-[0.08em] text-muted-foreground font-semibold">Secțiune</div>
              <div className="font-medium">{article.section}</div>
            </div>
          )}
          {article.doi && (
            <div>
      ...
      {(article.abstract_ro || article.abstract_en) && (
        <div className="rounded-lg border bg-card p-6 md:p-8 mb-6 shadow-sm">
          ...
          {article.abstract_en && article.abstract_en !== article.abstract_ro && (
            <>
      ...
      {keywords.length > 0 && (
        <div className="rounded-lg border bg-card p-6 mb-6 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {keywords.map((kw, i) => (
              <span key={i} className="px-3 py-1 rounded-sm text-sm bg-secondary border text-secondary-foreground">
                {kw}
              </span>
            ))}
          </div>
      ...
      <div className="flex gap-3">
        ...
        <Button disabled variant="outline" size="lg">
          <BookOpen className="mr-2 h-4 w-4" /> Citește online
        </Button>
      </div>
    </div>
  );
